...
const ProjectDetailPage = () => {
  ...
  if (!project) {
    return (
      <div className="text-center p-8">
        <p className="text-gray-500 dark:text-gray-300 mb-4">Project not found</p>
        <button
          onClick={() => navigate('/projects')}
          className="bg-blue-500 text-white px-4 py-2 rounded-lg hover:bg-blue-600"
        >
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 dark:from-gray-900 dark:to-gray-800 pb-12">
      {/* Project Header */}
      <div className="max-w-5xl mx-auto mt-8 mb-8">
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 flex flex-col md:flex-row md:items-center md:justify-between gap-6 border border-gray-100 dark:border-gray-700">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white mb-1 tracking-tight">{project.name}</h1>
              {project.description && <p className="text-gray-500 dark:text-gray-300 text-base mb-1 max-w-md line-clamp-2">{project.description}</p>}
              <div className="flex items-center text-sm text-gray-400 dark:text-gray-300 gap-2">
                <MdPerson />
      ...
      <div className="max-w-7xl mx-auto px-2 md:px-6">
        <div className="relative">
          ...
          <div className="w-full overflow-x-auto pb-4 rounded-xl bg-white/80 dark:bg-gray-800/80 shadow-inner">
            <div className="min-w-[900px] flex gap-8 py-8">
              {/* To Do (Unassigned) */}
              <div className="flex-1 bg-gray-50 dark:bg-gray-700 rounded-xl shadow-md flex flex-col max-h-[70vh] border border-gray-100 dark:border-gray-600">
                <div className="sticky top-0 z-10 bg-gray-100 dark:bg-gray-600 rounded-t-xl p-4 border-b border-gray-200 dark:border-gray-500">
                  <h3 className="text-lg font-bold text-gray-700 dark:text-gray-200 tracking-wide">To Do</h3>
                </div>
                <div className="flex-1 overflow-y-auto p-4 space-y-6">
                  {tasks.filter(t => !t.assignedUserId).length === 0 ? (
                    <div className="text-center text-gray-300 dark:text-gray-400 italic py-12">No open tasks</div>
                  ) : (
                    tasks.filter(t => !t.assignedUserId).map(task => (
                      <div key={task.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-5 flex flex-col gap-2 border border-gray-200 dark:border-gray-600 hover:shadow-2xl transition-shadow duration-200 group">
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            <h4 className="font-semibold text-gray-900 dark:text-white text-base mb-1 group-hover:text-blue-600 transition">{task.title}</h4>
                          ...
                          <div className="flex flex-col space-y-2 items-end ml-2">
                            <button onClick={() => handleViewTask(task)} className="text-gray-600 hover:text-gray-800 p-1 transition" title="View Details"><MdVisibility /></button>
              ...
              <div className="flex-1 bg-blue-50 dark:bg-blue-900/20 rounded-xl shadow-md flex flex-col max-h-[70vh] border border-blue-100 dark:border-blue-800">
                <div className="sticky top-0 z-10 bg-blue-100 dark:bg-blue-800 rounded-t-xl p-4 border-b border-blue-200 dark:border-blue-700">
                  <h3 className="text-lg font-bold text-blue-700 dark:text-blue-300 tracking-wide">In Progress</h3>
                </div>
                <div className="flex-1 overflow-y-auto p-4 space-y-6">
                  {tasks.filter(t => t.assignedUserId && t.status !== 'Done').length === 0 ? (
                    <div className="text-center text-blue-200 dark:text-blue-300 italic py-12">No tasks in progress</div>
                  ) : (
                    tasks.filter(t => t.assignedUserId && t.status !== 'Done').map(task => (
                      <div key={task.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-5 flex flex-col gap-2 border border-blue-200 dark:border-blue-600 hover:shadow-2xl transition-shadow duration-200 group">
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            <h4 className="font-semibold text-gray-900 dark:text-white text-base mb-1 group-hover:text-blue-700 transition">{task.title}</h4>
                            <p className="text-gray-500 dark:text-gray-300 text-sm mb-2 line-clamp-2">{task.description}</p>
                            <div className="flex flex-wrap gap-2 mb-1">
                              <span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(task.status)}`}>{task.status}</span>
                              {task.priority && <span className={`px-2 py-1 rounded-full text-xs font-medium border ${getPriorityColor(task.priority)} border-current`}><MdFlag className="inline mr-1" />{task.priority}</span>}
              ...
              {/* Done (Completed) */}
              <div className="flex-1 bg-green-50 dark:bg-green-900/20 rounded-xl shadow-md flex flex-col max-h-[70vh] border border-green-100 dark:border-green-800">
                <div className="sticky top-0 z-10 bg-green-100 dark:bg-green-800 rounded-t-xl p-4 border-b border-green-200 dark:border-green-700">
                  <h3 className="text-lg font-bold text-green-700 dark:text-green-300 tracking-wide">Done</h3>
                </div>
                <div className="flex-1 overflow-y-auto p-4 space-y-6">
                  {tasks.filter(t => t.status === 'Done').length === 0 ? (
                    <div className="text-center text-green-200 dark:text-green-300 italic py-12">No completed tasks</div>
                  ) : (
                    tasks.filter(t => t.status === 'Done').map(task => (
                      <div key={task.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-5 flex flex-col gap-2 border border-green-200 dark:border-green-600 hover:shadow-2xl transition-shadow duration-200 group">
                        <div className="flex justify-between items-start">
                          <div className="flex-1">
                            <h4 className="font-semibold text-gray-900 dark:text-white text-base mb-1 group-hover:text-green-700 transition">{task.title}</h4>
                            <p className="text-gray-500 dark:text-gray-300 text-sm mb-2 line-clamp-2">{task.description}</p>
                            <div className="flex flex-wrap gap-2 mb-1">
                              <span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(task.status)}`}>{task.status}</span>
                              {task.priority && <span className={`px-2 py-1 rounded-full text-xs font-medium border ${getPriorityColor(task.priority)} border-current`}><MdFlag className="inline mr-1" />{task.priority}</span>}
