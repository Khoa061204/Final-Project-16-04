...
const CalendarPage = () => {
  ...
  const fetchCalendarData = async () => {
    try {
      ...
      console.log('Calendar data received:', { eventsData, projectsData, tasksData });

      // Handle different response formats and ensure we're setting arrays
      const finalEvents = Array.isArray(eventsData) ? eventsData : (eventsData?.events || []);
      const finalProjects = Array.isArray(projectsData) ? projectsData : (projectsData?.projects || []);
      const finalTasks = Array.isArray(tasksData) ? tasksData : (tasksData?.tasks || []);

      console.log('Final arrays:', { finalEvents, finalProjects, finalTasks });
  ...
  const getEventsForDate = (date) => {
    ...
    const allEvents = [
      ...
      ...projectsArray.filter(project => project && project.dueDate).map(project => ({
        id: project.id,
        title: project.name,
      ...
      ...tasksArray.filter(task => task && task.dueDate).map(task => ({
        id: task.id,
        title: task.title,
    ...
    console.log('Filtered events for date:', dateStr, filteredEvents);
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
            ...
            <div className="grid grid-cols-7 bg-gray-50 dark:bg-gray-700">
              {['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(day => (
                <div key={day} className="px-4 py-3 text-center text-sm font-medium text-gray-500 dark:text-gray-300">
                  {day}
                </div>
              ))}
            </div>
      ...
      {showEventModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-96">
            <h2 className="text-xl font-semibold mb-4 text-gray-900 dark:text-white">Add Event</h2>
