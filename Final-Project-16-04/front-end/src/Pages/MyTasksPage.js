      ...
      {loading ? (
        ...
        <div className="flex gap-8">
          {['To Do', 'In Progress', 'Done'].map(status => (
            <div key={status} className="flex-1 bg-gray-50 rounded-xl shadow-md flex flex-col max-h-[70vh] border border-gray-100">
              <div className="sticky top-0 z-10 bg-gray-100 rounded-t-xl p-4 border-b border-gray-200">
                <h3 className="text-lg font-bold text-gray-700 tracking-wide">{status}</h3>
              </div>
              <div className="flex-1 overflow-y-auto p-4 space-y-6">
                {grouped[status].length === 0 ? (
                  <div className="text-center text-gray-300 italic py-12">No tasks</div>
                ) : grouped[status].map(task => (
                  <div key={task.id} className="bg-white rounded-xl shadow-lg p-5 flex flex-col gap-2 border border-gray-200">
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        <h4 className="font-semibold text-gray-900 text-base mb-1">{task.title}</h4>
                        ...
                        <div className="text-xs text-gray-400">Project: {task.project?.name}</div>
                      </div>
                    </div>
                  </div>
                ))}
